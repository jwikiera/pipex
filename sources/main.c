...
int	main(int argc, char *argv[])
{
	...
	if (ft_file_exists(pipex->commands[pipex->commandc - 1][0])
		&& !ft_command_ex_current(pipex->commands[pipex->commandc - 1][0],
							environ))
		ret = 126;
	else if (!ft_command_ex_current(pipex->commands[pipex->commandc - 1][0],
		environ))
		ret = 127;
	if (pipex->file2_fd == -1)
		ret = 1;
